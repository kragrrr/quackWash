...
Promise.all([fetchStop('2500354'), fetchStop('2500122'), fetchStop('250010'), fetchStop('250019')]).then(results => {
    results.forEach((r, i) => {
        ...
        if (stops.length > 0) {
            console.log(`Stop ${stopId} first bus:`, {
                number: stops[0].transportation.number,
                uid: stops[0].transportation.uid,
                id: stops[0].transportation.id,
                properties: stops[0].transportation.properties
            });
            console.log(stops[0]);
        }
    });
